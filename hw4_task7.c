...
/*
 ...
 Homework #: 4
 To Compile: mpicc -g -Wall -std=c99 -o hw4_task7 hw4_task7.c
 To Run: hw4_task7 5000 5000 1 outputs/output.txt
*/
...
int main(int argc, char *argv[]) {
    ...
    while (generation < maxGenerations && global_changed) {
        ...
        MPI_Irecv(current[0], local_board_cols, MPI_INT, up, 1, MPI_COMM_WORLD, &requests[0]); // Receive from up
        MPI_Irecv(current[local_board_rows - 1], local_board_cols, MPI_INT, down, 0, MPI_COMM_WORLD, &requests[1]); // Receive from down

        // Start non-blocking sends
        MPI_Isend(current[1], local_board_cols, MPI_INT, up, 0, MPI_COMM_WORLD, &requests[2]); // Send to up
        MPI_Isend(current[local_board_rows - 2], local_board_cols, MPI_INT, down, 1, MPI_COMM_WORLD, &requests[3]); // Send to down
